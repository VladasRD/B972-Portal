...
@Component({
  selector: 'app-report-tqa-list',
  templateUrl: './report-tqa-list.component.html',
  styleUrls: ['./report-tqa-list.component.css']
})
export class ReportTqaListComponent extends GrudList<Report> implements OnInit {
  form: FormGroup;
  ...
  currentProject = ProjectEnum.B981;
  ...
  constructor(
    ...
    this.form.get('deviceFilter').valueChanges.subscribe(val => {
      if (this.currentProject === ProjectEnum.B981) {
        this.displayedColumns = ['data', 'hour', 'temperatura', 'ph', 'fluor', 'cloro', 'turbidez', 'rele1', 'rele2', 'rele3', 'rele4', 'rele5', 'rele6', 'rele7'];
      } else {
  ...
  exportToExcel(): void {
    ...
    this.sgiService.exportReportsToExcel(this.deviceFilter, startPeriod != null ? startPeriod.toJSON() : <string>null, endPeriod != null ? endPeriod.toJSON() : <string>null, 2000)
    .subscribe(data => {
      Utils.fileDownload(data, 'relatório-tqa.xlsx');
      this.isExporting = false;
    },
    err => {
      this.messageService.addError(`Erro na exportação do relatório.`);
    }
    );
  }
