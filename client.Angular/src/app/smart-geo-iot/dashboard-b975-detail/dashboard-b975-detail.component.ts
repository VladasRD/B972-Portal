...
@Component({
  selector: 'app-dashboard-b975-detail',
  templateUrl: './dashboard-b975-detail.component.html',
  styleUrls: ['./dashboard-b975-detail.component.css']
})
export class DashboardB975DetailComponent implements OnInit {
  ...
  get pageTitle(): string {
    return String.Format('{0}', this.dashboard.name);
  }
  ...
  get bgStatusDJ(): string {
    if (this.dashboard.b975.statusDJ === 'Aguardando') {
        return EstadoDJ.enumBackground[0];    
    }
    if (this.dashboard.b975.statusDJ === 'Operacional') {
        return EstadoDJ.enumBackground[1];  
    }
    if (this.dashboard.b975.statusDJ === 'Em carência') {
        return EstadoDJ.enumBackground[2];  
    }
    if (this.dashboard.b975.statusDJ === 'Em Ciclos') {
        return EstadoDJ.enumBackground[3];  
    }
    if (this.dashboard.b975.statusDJ === 'Em bloqueio') {
  ...
  private getDashboard(): void {
    ...
    this.sgiService.getDashboard(this._deviceId, _dateFilter != null ? _dateFilter.toJSON() : <string>null, this.seqNumber, this.navigation, 0, ProjectEnum.B975).subscribe(d => {
  ...
  get latitude() {
    return this.dashboard.b975 ? this.dashboard.b975.latitude : 0;
  }

  get longitude() {
    return this.dashboard.b975 ? this.dashboard.b975.longitude : 0;
  }

  get radius() {
    return this.dashboard.b975 ? Number(this.dashboard.b975.radius) : 0;
  }
  ...
  editValueFields(field: string, value: string) {
    ...
    this.sgiService.changeFieldsTRM11(this.deviceId, field, value)
      .subscribe(() => {
        this.updateData()
        this.messageService.add(`Alteração dos dados efetuada com sucesso.`);
      },
        err => {
